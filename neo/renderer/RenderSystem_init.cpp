...
idCVar r_rtShadowBlurStaggerNear( "r_rtShadowBlurStaggerNear", "320", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlurStagger: lights whose origin is nearer to the viewer than this (world units) refresh every frame; beyond it every 2nd frame. 0 = every light is at least half rate", 0.0f, 8192.0f );
idCVar r_rtShadowBlurStaggerFar( "r_rtShadowBlurStaggerFar", "800", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlurStagger: lights whose origin is farther from the viewer than this (world units) refresh every 3rd frame. 0 = every light at a third of the rate", 0.0f, 16384.0f );
