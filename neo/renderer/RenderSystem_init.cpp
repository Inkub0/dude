...
idCVar r_rtShadowBlur( "r_rtShadowBlur", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "soften the hard ray-traced shadows (r_rtSunShadows, r_rtMovingLights) with a screen-space blur (docs/rtx-shadow-blur.md, Vulkan + RT hardware): the same one ray per pixel, traced per light right before it draws, then blurred by an amount that follows the ray's occluder distance - sharp at contact, softer with the gap, at most 32 pixels. Deterministic: no noise, nothing accumulated over frames. 0 = the hard shadows exactly as before. Which lights are ray-served is unchanged; needs the depth/normal prepass (SSAO, RTAO or motion vectors on), else the lights keep their hard ray" );
idCVar r_rtShadowBlurIntensity( "r_rtShadowBlurIntensity", "1.0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlur: how strongly the ray-traced shadows are blurred. Scales the blur width everywhere; the width itself still follows the geometry (none where the caster touches the surface, more as the gap grows) and is capped at 32 pixels, so high values mostly soften the near-contact edges further. 0 = no blur (the hard shadow), 1 = default", 0.0f, 4.0f );
