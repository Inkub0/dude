...
idCVar r_shadowMapSplitDynDrop( "r_shadowMapSplitDynDrop", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "cube-shadow split cache: render the dynamic (movers-only) cube this many resolution tiers below the static cube (0 = same size, 1 = half, 2 = quarter). The movers' cube re-renders every frame, so a smaller one cuts that per-frame fill cost; moving shadows get proportionally softer (PCF kernel and depth bias scale to the coarser texels). Floors at the smallest pool tier. NOT fidelity-identical above 0 - opt-in perf lever (docs/shadow-research.md, 2026-08 recency check)", 0, 2 );
