...
idCVar r_emissiveLightRadius( "r_emissiveLightRadius", "1.80", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "reach of GUI-screen fill lights, as a multiple of the screen's own size", 0.25f, 16.0f );
idCVar r_emissiveLightFalloff( "r_emissiveLightFalloff", "0.5", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "how gradually a screen fill light fades along its cone: 0 = stays bright then drops off sharply near its reach, 1 = fades gently almost from the screen. 0.5 = default (matches the shipped look)", 0.0f, 1.0f );
