...
idCVar r_shadowMapCacheDebug( "r_shadowMapCacheDebug", "0", CVAR_RENDERER | CVAR_BOOL, "print a once/sec cube-shadow cache breakdown: hit rate + cube re-renders split by cause (cold = new/evicted light, warm-caster = an occluder moved, warm-light = the light moved), plus scratch/dynamic/deferred/evictions. Dev diagnostic for shadow-cache work" );
