...
idCVar r_rtShadowBlurIntensity( "r_rtShadowBlurIntensity", "1.0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlur: how strongly the ray-traced shadows are blurred. Scales the blur width everywhere; the width itself still follows the geometry (none where the caster touches the surface, more as the gap grows) and is capped at 16 pixels, so high values mostly soften the near-contact edges further. 0 = no blur (the hard shadow), 1 = default", 0.0f, 4.0f );
