...
idCVar r_rtShadowBlurIntensity( "r_rtShadowBlurIntensity", "1.0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlur: how strongly the ray-traced shadows are blurred. Scales the blur width everywhere; the width itself still follows the geometry (none where the caster touches the surface, more as the gap grows) and is capped at 24 pixels, so high values mostly soften the near-contact edges further. 0 = no blur (the hard shadow), 1 = default", 0.0f, 4.0f );
idCVar r_rtShadowBlurCurve( "r_rtShadowBlurCurve", "1.0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlur: shape of the progressive blur - how fast a shadow softens as it gets further from the object casting it. Width = intensity * g^curve, where g = (caster-to-surface distance) / (caster-to-light distance). 1 = geometric growth (what a real light does); below 1 = softens sooner and then levels off (more uniform blur); above 1 = stays crisp for longer near the object, then widens fast", 0.25f, 4.0f );
