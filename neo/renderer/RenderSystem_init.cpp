...
idCVar r_tessSeamFade( "r_tessSeamFade", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "how strongly r_tessDisplace is faded out at UV seams. Displacement height is read from the bump map at the vertex UV, and the two halves of a UV seam are coincident vertices with DIFFERENT UVs -- they sample different heights and slide apart, which is what opens the gaps around hands and shoulders. 1 = pin seam vertices (no gap, slightly less relief in a narrow band around the seam), 0 = old behaviour", 0.0f, 1.0f );
