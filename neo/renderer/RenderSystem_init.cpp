...
idCVar r_rtShadowBlur( "r_rtShadowBlur", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "soften the hard ray-traced shadows (r_rtSunShadows, r_rtMovingLights) with a screen-space blur (docs/rtx-shadow-blur.md, Vulkan + RT hardware): the same one ray per pixel, traced per light right before it draws, then blurred by an amount that follows the ray's occluder distance - sharp at contact, softer with the gap, at most 16 pixels. Deterministic: no noise, nothing accumulated over frames. 0 = the hard shadows exactly as before. Which lights are ray-served is unchanged; needs the depth/normal prepass (SSAO, RTAO or motion vectors on), else the lights keep their hard ray" );
idCVar r_rtShadowBlurLightSize( "r_rtShadowBlurLightSize", "3", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlur: radius (world units) of the sphere standing in for a point / projected light - Doom 3 lights are dimensionless points, so this IS the softness. Penumbra half-width = radius * occluderDist / (lightDist - occluderDist). Point lights scale it by max(1, largest light_radius axis / 256)", 0.0f, 64.0f );
idCVar r_rtShadowBlurSunAngle( "r_rtShadowBlurSunAngle", "1.0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlur: angular radius in degrees of parallel (sun) lights. The real sun is ~0.27; larger reads softer", 0.0f, 10.0f );
idCVar r_rtShadowBlurDebug( "r_rtShadowBlurDebug", "0", CVAR_RENDERER | CVAR_INTEGER, "r_rtShadowBlur diagnostics: 1 = trace the per-light ray pass but skip tiles + blur (hard mask) - the r_vkGpuTime difference to 0 is the blur's own cost, the difference to r_rtShadowBlur 0 is the ray pass's; 2 = once-per-second console readout of how many lights are blurred per view and how much screen their rects cover - both the blurred rects (projected bounds of the lit surfaces) and the lights' full scissor rects", 0, 2, idCmdSystem::ArgCompletion_Integer<0,2> );
