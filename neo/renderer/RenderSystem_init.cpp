...
// A/B experiment (docs/ssao-perf-optimization.md Phase 4): fund a smaller horizon-search
// budget with temporal accumulation instead of brute-force sampling. Not archived, not
// preset-wired — flip it live against the Ultra/Nightmare budgets and compare; promoted
// into the presets (or removed) once the verdict is in.
idCVar r_ssaoTemporalTrade( "r_ssaoTemporalTrade", "0", CVAR_RENDERER | CVAR_BOOL, "A/B toggle: 1 = SSAO runs 4 slices x 6 steps with temporal accumulation forced ON (overriding r_ssaoSlices/Steps/Temporal), the cheaper temporal-funded budget; 0 = the current cvars. On Ultra/Nightmare the temporal reprojection rides the per-object velocity buffer, so moving objects track better than Medium's camera-only temporal" );
