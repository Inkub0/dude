...
idCVar r_rtShadowBlurStagger( "r_rtShadowBlurStagger", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "r_rtShadowBlur perf: lights far from the viewer refresh their blurred shadow mask less often - nearer than r_rtShadowBlurStaggerNear every frame, up to r_rtShadowBlurStaggerFar every 2nd frame, beyond that every 3rd (distance = viewer to the light's VOLUME, 0 when standing inside it). A reused mask is looked up reprojected through the camera it was made with, and any pixel it cannot answer for (hidden or off screen back then) traces a hard ray for that frame. Costs one view-sized RGBA8 mask of VRAM per staggered light (~15 MB at 1440p). Moving casters' shadows update at the light's reduced rate; a light that itself moves always refreshes" );
idCVar r_rtShadowBlurStaggerNear( "r_rtShadowBlurStaggerNear", "256", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlurStagger: lights whose volume is nearer than this (world units) refresh every frame. 0 = even lights you stand inside are staggered", 0.0f, 8192.0f );
idCVar r_rtShadowBlurStaggerFar( "r_rtShadowBlurStaggerFar", "768", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "r_rtShadowBlurStagger: lights whose volume is farther than this (world units) refresh every 3rd frame; between Near and Far every 2nd", 0.0f, 16384.0f );
