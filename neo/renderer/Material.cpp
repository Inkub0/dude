...
void idMaterial::CommonInit() {
	...
	// DUDE PBR (docs/pbr-materials.md Phase B): -1 = no table entry
	pbrMetalness = -1.0f;
	pbrRoughness = -1.0f;
...
===============================================================================
	...
	Line format: "<material name> <metalness> <roughness>", '#' starts a comment.
	Keys are matched case-insensitively. The reloadPbrTable console command
	(RenderSystem_init.cpp) invalidates the cache and re-applies to parsed
	materials, so override edits can be tuned live in-game.
...
struct pbrTableEntry_t {
	float	metalness;
	float	roughness;
	int		category;		// pbrCategory_t; PBR_CAT_NONE for override-file entries
};
...
static void PBR_LoadTableFile( const char *path, bool isOverride ) {
	char *buf = NULL;
	...
	while ( *p ) {
		...
		char cat[32];
		float m, r;
		int fields = sscanf( line.c_str(), "%255s %f %f %31s", name, &m, &r, cat );
		if ( fields >= 3 ) {
			pbrTableEntry_t e;
			e.metalness = idMath::ClampFloat( 0.0f, 1.0f, m );
			e.roughness = idMath::ClampFloat( 0.03f, 1.0f, r );
			// override-file entries are explicit: never superseded by the
			// per-category sliders
			e.category = ( !isOverride && fields == 4 ) ? PBR_CategoryForName( cat ) : PBR_CAT_NONE;
			idStr key( name );
			key.ToLower();
...
static void PBR_EnsureTableLoaded( void ) {
	...
	pbrTableLoaded = true;
	pbrTable.Clear();
	PBR_LoadTableFile( "pbr/pbr_materials.cfg", false );
	PBR_LoadTableFile( "pbr/pbr_overrides.cfg", true );
}
