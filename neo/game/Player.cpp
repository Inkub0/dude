...
void idPlayer::CalculateFirstPersonView( void ) {
	...
	// mouselook, even fast flicks, stays under this threshold and is interpolated.
...
Nudges the cached first-person view toward the sub-tic time so player motion looks smooth above
60 fps, while the simulation keeps stepping at the fixed tic rate. frac is the interpolation alpha
in [0,1] between the previous and current tic (see common->GetTicInterpolation()).

This interpolates between two already-simulated states, so it renders up to one tic (~16.7ms) in
the past; it never extrapolates, avoiding overshoot on direction changes. Aim is affected too, but
shooting is resolved from the usercmd at the game tic, so no shot latency is added.
==================
*/
void idPlayer::InterpolateRenderView( float frac ) {
	...
	// (INFLUENCE_LEVEL3, +/-1 deg/tic) or OVERRIDES pitch (centerView auto-recenter); in any of those the
	// overlay would drift / rubber-band against the sim and snap each tic, so defer to the whole-axis
	// slerp. Keep this mirrored with idPlayer::UpdateViewAngles.
	const bool viewAnglesFree = ( !objectiveSystemOpen
...
Forwards render interpolation to the view model so the gun tracks the interpolated view.
==================
*/
