// DUDE tessellation (docs/tessellation.md): generic (blended material / decal)
// evaluation shader. Same PN evaluation, domain and winding as interaction.tese /
// zfill.tese so:
//   - an on-body blend stage that must survive depth-EQUAL against the (displaced)
//     zfill prepass — e.g. the imp/zombie burning-corpse ember, a self-illum add,
//     a mod emissive — displaces with the SAME bump texel, seam mask and strength as
//     zfill.tese and therefore lands on the sealed depth instead of failing EQUAL
//     everywhere the surface is displaced (which left the ember only on flat patches
//     like the soles of the feet);
//   - a blood-overlay decal — a separate surface with no bump stage — is bound
//     flatNormalMap on unit 1, so dudeTessDisplace sees relief 0 and it merely
//     PN-follows the deformed base with no normal-map push (unchanged behaviour).

#include "renderparms.glsl"
#include "tess.glsl"

SAMPLER_BINDING(1) uniform sampler2D u_bumpMap;	// displacement source (flatNormalMap => no push)

layout(triangles, fractional_odd_spacing, cw) in;	// fractional = smooth LOD:
										// new verts slide in as the distance factor changes instead of popping.
										// Must match in every pass (crack-free + depth-EQUAL).

VARY(0) in vec2 i_TexCoord[];
VARY(1) in vec4 i_Color[];
VARY(2) in vec3 i_ModelPos[];
VARY(3) in vec4 i_ModelNormal[];		// .w = UV-seam displacement mask
VARY(4) in vec2 i_TexBump[];

VARY(0) out vec2 var_TexCoord;
VARY(1) out vec4 var_Color;

void main() {
	vec3 tc = gl_TessCoord;

	// PN wants unit corner normals; .w is the UV-seam displacement mask (tess.glsl)
	vec3 n0 = normalize( i_ModelNormal[0].xyz );
	vec3 n1 = normalize( i_ModelNormal[1].xyz );
	vec3 n2 = normalize( i_ModelNormal[2].xyz );

	vec3 pos = dudeTessPN( i_ModelPos[0], i_ModelPos[1], i_ModelPos[2], n0, n1, n2, tc );

	var_TexCoord = i_TexCoord[0] * tc.x + i_TexCoord[1] * tc.y + i_TexCoord[2] * tc.z;
	var_Color    = i_Color[0]    * tc.x + i_Color[1]    * tc.y + i_Color[2]    * tc.z;

	// must match zfill.tese / interaction.tese exactly (depth-EQUAL): same PN position,
	// same displacement along the same interpolated geometric normal with the same bump
	// texel. Bump-less surfaces bind flatNormalMap => relief 0 => no displacement.
	vec2 texBump = i_TexBump[0] * tc.x + i_TexBump[1] * tc.y + i_TexBump[2] * tc.z;
	vec3 geoN = normalize( n0 * tc.x + n1 * tc.y + n2 * tc.z );
	float seam = i_ModelNormal[0].w * tc.x + i_ModelNormal[1].w * tc.y + i_ModelNormal[2].w * tc.z;
	pos = dudeTessDisplace( pos, geoN, u_bumpMap, texBump, seam );

	gl_Position = u_mvpMatrix * vec4( pos, 1.0 );
}
