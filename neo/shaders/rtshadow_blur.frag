...
// the texel through). Otherwise (1) which shadowed neighbours REACH me - a neighbour x pixels away
...
SAMPLER_BINDING(1) uniform sampler2D u_tiles;		// R = 1: a shadow edge is within reach of this 8x8 tile

VARY(0) in vec2 var_TexCoord;

layout(location = 0) out vec4 fragColor;

const int   K         = 16;		// taps per side = the widest penumbra half-width, in pixels
...
void main() {
	...
	if ( !( c.b > 0.0 ) || texelFetch( u_tiles, ip >> 3, 0 ).r < 0.5 ) {
		return;
	}
	...
	// one sweep of the row/column: every tap is fetched once and serves both steps
	vec4 tap[2 * K + 1];
	for ( int i = -K; i <= K; i++ ) {
		ivec2 q = ip + axis * i;
		tap[i + K] = ( q.x < 0 || q.y < 0 || q.x > hi.x || q.y > hi.y ) ? vec4( 1.0, 0.0, 0.0, 0.0 )
		                                                               : texelFetch( u_src, q, 0 );
	}
	...
	float dA = tap[K + 1].b, dB = tap[K - 1].b;
