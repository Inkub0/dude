# Depth linearisation follows the view's projection (cinematics cram the near plane)
...
(`R_SetupProjection`, tr_main.cpp: `P[10] = -0.999`, `P[14] = -2 * zNear`). **Cinematics set
`renderView.cramZNear`, which multiplies the near plane by 0.25** (stock behaviour, so close-up
cutscene cameras don't clip through faces). With the play-time pair, every view distance computed
during a cutscene came out **4x too large**.
...
- **SSAO / GTAO**: the scene read 4x larger, so the world-space AO radius covered a quarter of the
  intended neighbourhood - weaker, tighter AO.
- **RTAO**: rays started 4x too far along the view ray - behind the surfaces.
- **SSR / RT reflections**: ray origins and depth comparisons in a 4x-scaled space, reprojected
  with the true matrix.
- **Soft particles**: scene-vs-particle distance 4x too large, so the soft fade was 4x shorter.

## Not changed

- Raw-depth thresholds such as the sky cut `raw >= 0.9994` (about 30000 units in play, about 7500
  with the crammed near plane - still far beyond any cutscene set).
